    ...
    private function addMatchSection(NodeBuilder $rules)
    {
        $rules
            ->arrayNode('match')
                ...
                ->fixXmlConfig('method')
                ->validate()
                    ->ifTrue(function ($v) {return !empty($v['additional_safe_status']) && !empty($v['match_response']);})
                    ->thenInvalid('You may not set both additional_safe_status and match_response')
                ->end()
                ->children()
                    ->scalarNode('path')
                        ->defaultNull()
                    ...
                    ->end()
                    ->arrayNode('additional_safe_status')
                        ->prototype('scalar')->end()
                        ->info('Additional response HTTP status codes that will get the headers of this rule.')
                    ->end()
                    ->scalarNode('match_response')
                        ->defaultValue(array())
                        ->info('Expression to decide whether response should be matched. Replaces HTTP code check and additional_safe_status.')
                    ->end()
                ->end()
            ->end()
        ;
